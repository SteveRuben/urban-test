...
class SubscriptionService {
  ...
  async getActiveSubscription(): Promise<Subscription> {
    try {
      ...
      if (subscription) {
        subscription.startDate = subscription.startDate ? new Date(subscription.startDate) : null;
        subscription.endDate = subscription.endDate ? new Date(subscription.endDate) : null;
        subscription.currentPeriodStart = subscription.currentPeriodStart ? new Date(subscription.currentPeriodStart) : null;
        subscription.currentPeriodEnd = subscription.currentPeriodEnd ? new Date(subscription.currentPeriodEnd) : null;
        subscription.createdAt = subscription.createdAt ? new Date(subscription.createdAt) : null;
        subscription.updatedAt = subscription.updatedAt ? new Date(subscription.updatedAt) : null;
      }
